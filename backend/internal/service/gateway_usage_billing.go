...
func (s *GatewayService) getUserGroupRateMultiplier(ctx context.Context, userID, groupID int64, groupDefaultMultiplier float64) float64 {
	return s.getUserGroupRateResolution(ctx, userID, groupID, groupDefaultMultiplier).Multiplier
}

func (s *GatewayService) getUserGroupRateResolution(ctx context.Context, userID, groupID int64, groupDefaultMultiplier float64) UserGroupRateResolution {
	if s == nil {
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput) error {
	...
	if apiKey.GroupID != nil && apiKey.Group != nil {
		...
		groupRate := s.getUserGroupRateResolution(ctx, user.ID, *apiKey.GroupID, groupDefault)
		multiplier = resolveAccountUserBillingMultiplier(account, groupRate)
	}
	...
	if apiKey.Group != nil && apiKey.Group.Platform == PlatformComposite {
		billingModel = s.compositeBillableModel(ctx, apiKey, account, billingModel, concreteBillingModel)
	}
	// 通用兜底（与 OpenAI 路径的 usageBillingModelCandidates 语义对齐）：
	// 选定模型查不到任何价格时回退到实际转发的具体模型。已定价流量不受影响。
	billingModel = s.billableModelWithFallback(ctx, apiKey, account, billingModel, result.UpstreamModel, result.Model)
	...
	if responseModel := responseModelBillingDeclaration(
		...
		if identified, responseChannelPriced := s.hasIdentifiedResponseModelPricing(ctx, responseModel, apiKey, account); identified {
			responseCost := s.calculateRecordUsageCost(ctx, result, apiKey, account, responseModel, multiplier, imageMultiplier, pricingAt)
			baselineChannelPriced := s.resolveChannelPricing(ctx, billingModel, apiKey, account) != nil
...
func (s *GatewayService) calculateRecordUsageCost(
	...
	if result.ImageCount > 0 {
		if resolved := s.resolveChannelPricing(ctx, billingModel, apiKey, account); resolved != nil && resolved.Mode == BillingModeToken {
			return s.calculateTokenCost(ctx, result, apiKey, account, billingModel, multiplier, pricingAt)
		}
		return s.calculateImageCost(ctx, result, apiKey, account, billingModel, imageMultiplier)
	}

	// Voice audio (TTS / STT / realtime) when present on the forward result.
	if result.AudioUsage != nil {
		if resolved := s.resolveChannelPricing(ctx, billingModel, apiKey, account); resolved != nil &&
			resolved.Mode == BillingModePerRequest {
			gid := apiKey.Group.ID
			cost, err := s.billingService.CalculateCostUnified(CostInput{
				Ctx: ctx, Model: billingModel, GroupID: &gid, Group: apiKey.Group, Account: account,
...
func (s *GatewayService) compositeBillableModel(ctx context.Context, apiKey *APIKey, account *Account, billingModel, concreteBillingModel string) string {
	if concreteBillingModel == "" || billingModel == concreteBillingModel {
		return billingModel
	}
	if s.resolveChannelPricing(ctx, billingModel, apiKey, account) != nil {
		return billingModel
	}
...
func (s *GatewayService) billableModelWithFallback(ctx context.Context, apiKey *APIKey, account *Account, billingModel string, fallbacks ...string) string {
	if s.hasResolvableTokenPricing(ctx, billingModel, apiKey, account) {
		return billingModel
	}
	for _, fallback := range fallbacks {
		...
		if s.hasResolvableTokenPricing(ctx, fallback, apiKey, account) {
...
func (s *GatewayService) hasResolvableTokenPricing(ctx context.Context, model string, apiKey *APIKey, account *Account) bool {
	if strings.TrimSpace(model) == "" {
		return false
	}
	if s.resolveChannelPricing(ctx, model, apiKey, account) != nil {
		return true
	}
...
func (s *GatewayService) hasIdentifiedResponseModelPricing(ctx context.Context, model string, apiKey *APIKey, account *Account) (identified bool, channelPriced bool) {
	if strings.TrimSpace(model) == "" {
		return false, false
	}
	if s.resolveChannelPricing(ctx, model, apiKey, account) != nil {
...
func (s *GatewayService) resolveChannelPricing(ctx context.Context, billingModel string, apiKey *APIKey, account *Account) *ResolvedPricing {
	if s.resolver == nil || apiKey.Group == nil {
		return nil
	}
	gid := apiKey.Group.ID
	resolved := s.resolver.Resolve(ctx, PricingInput{Model: billingModel, GroupID: &gid, Group: apiKey.Group, Account: account})
	if resolved.Source == PricingSourceAccount || resolved.Source == PricingSourceGroup || resolved.Source == PricingSourceChannel {
		return resolved
	}
	return nil
}
...
func (s *GatewayService) calculateImageCost(
	...
	resolved := s.resolveChannelPricing(ctx, billingModel, apiKey, account)
	if resolved != nil && (resolved.Source == PricingSourceAccount || resolved.Source == PricingSourceGroup) {
		gid := apiKey.Group.ID
		cost, err := s.billingService.CalculateCostUnified(CostInput{
			Ctx: ctx, Model: billingModel, GroupID: &gid, Group: apiKey.Group, Account: account,
