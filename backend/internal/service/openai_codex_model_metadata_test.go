...
func TestAstraCodexToolCapabilitiesUseAccountScopeAndSharedDeclarations(t *testing.T) {
	...
	// 本地设计：路由只认显式 mode（probe 标记仅诊断）。
	// chat bridge 账号显式配置 force_chat_completions。
	bridge.Extra = map[string]any{"openai_responses_mode": "force_chat_completions"}
...
func TestBuildCodexModelsManifestForGroupAdvertisesSearchOnlyForChatBridgeRoutes(t *testing.T) {
	...
	// 本地设计：路由只认显式 mode（probe 标记仅诊断）。
	// chat bridge 账号显式 force_chat_completions，原生 Responses 账号显式 force_responses。
	newAccount := func(id int64, nativeResponses bool) Account {
		...
		return Account{
			...
			Extra: map[string]any{"openai_responses_mode": mode},
		}
	}
