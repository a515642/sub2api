...
type UpdateGroupRequest struct {
	...
	CodexModelsManifestConfig *service.GroupCodexModelsManifestConfig `json:"codex_models_manifest_config"`
	// 自定义 /v1/models 展示列表（本地定制功能）；nil 表示未提供不改动
	ModelsListConfig *service.GroupModelsListConfig `json:"models_list_config"`
	// 分组 RPM 上限（0 = 不限制）；nil 表示未提供不改动
	RPMLimit *int `json:"rpm_limit"`
	// OpenAI API Key 号池首 Token 超时切换配置；nil 表示不修改
	FirstOutputFailoverEnabled        *bool `json:"first_output_failover_enabled"`
	FirstOutputFailoverTimeoutSeconds *int  `json:"first_output_failover_timeout_seconds"`
	FirstOutputFailoverMaxSwitches    *int  `json:"first_output_failover_max_switches"`
...
func (h *GroupHandler) Create(c *gin.Context) {
	...
	group, err := h.adminService.CreateGroup(c.Request.Context(), &service.CreateGroupInput{
		...
		FirstOutputFailoverEnabled:     req.FirstOutputFailoverEnabled,
		FirstOutputFailoverTimeoutSeconds: req.FirstOutputFailoverTimeoutSeconds,
		FirstOutputFailoverMaxSwitches: req.FirstOutputFailoverMaxSwitches,
