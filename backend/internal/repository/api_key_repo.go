	...
	m, err := r.activeQuery().
		...
		WithGroup(func(q *dbent.GroupQuery) {
			q.Select(
				...
				group.FieldFirstOutputFailoverEnabled,
				group.FieldFirstOutputFailoverTimeoutSeconds,
				group.FieldFirstOutputFailoverMaxSwitches,
				...
				// models_list_config 是本地定制的 /v1/models 展示列表配置，
				// 与 model_allowlist 相互独立；展示门读取认证快照，漏选会让
				// 展示功能在热路径静默失效。
...
func groupEntityToService(g *dbent.Group) *service.Group {
	...
	return &service.Group{
		...
		FirstOutputFailoverEnabled:        g.FirstOutputFailoverEnabled,
		FirstOutputFailoverTimeoutSeconds: g.FirstOutputFailoverTimeoutSeconds,
		FirstOutputFailoverMaxSwitches:    g.FirstOutputFailoverMaxSwitches,
