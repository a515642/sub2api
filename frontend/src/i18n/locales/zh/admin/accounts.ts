export default {
  accounts: {
    ...
    upstreamBilling: {
      ...
      autoProbe: '自动探测上游声明成本倍率',
      autoProbeHint: '启用后按全局周期刷新上游声明成本倍率；此开关本身不会修改账号成本倍率。',
      syncRate: '同步上游声明成本倍率',
      syncRateHint: '成功探测后自动更新账号成本倍率，同步的是不含高峰的基准成本倍率；探测失败或声明超出允许范围时保持不变。开启本项会同时打开“自动探测上游声明成本倍率”。',
      syncRateManagedHint: '当前账号成本倍率由上游声明的基准成本倍率（不含高峰）自动维护。',
      syncedRateTooltip: '该账号成本倍率由上游声明的基准成本倍率（不含高峰）自动同步',
      manualProbe: '立即探测上游成本倍率',
      stale: '已过期',
      unsupported: '不支持',
      failed: '失败',
      notProbed: '未探测',
    ...
    bulkEdit: {
      ...
      rateSyncWarning: '已开启上游成本倍率同步的账号不能批量手工修改账号成本倍率，请先在账号编辑页关闭同步。',
      rateSyncConflict: '无法修改账号成本倍率：{count} 个目标账号已开启上游成本倍率同步。',
    ...
    openai: {
      ...
      responsesMode: '上游文本协议',
      responsesModeDesc:
        '选择请求转换后发送到上游的固定协议：Responses、Chat Completions，或仅媒体端点。仅媒体端点适用于只提供视频/图片能力、不提供文本协议的账号。',
      responsesModeRequired: '请为此 OpenAI API Key 选择上游文本协议或“仅媒体端点”。',
      responsesModeForceResponses: '强制 Responses',
      responsesModeForceChatCompletions: '强制 Chat Completions',
      responsesModeMediaOnly: '仅媒体端点（视频/图片）',
      responsesModeTextDisabledHint: '此配置只决定上游协议；是否参与文本请求调度由下方“可调度能力”控制。',
      ...
      endpointCapabilities: '可调度能力',
      endpointCapabilitiesDesc:
        '控制账号是否参与对应请求资格的调度。文本生成仅表示账号可承接文本生成请求资格，不代表具体协议选择；Embeddings 控制 /v1/embeddings 文本向量化端点资格。',
      capabilityTextGeneration: '文本生成',
      capabilityEmbeddings: 'Embeddings',
      capabilityVideoGeneration: '视频生成（/v1/videos）',
      responsesStatusUnconfigured: '尚未配置上游协议',
      responsesStatusForcedResponses: '已强制 Responses',
      responsesStatusForcedChatCompletions: '已强制 Chat Completions',
      responsesStatusMediaOnly: '仅媒体端点',
      planType: '订阅档位（手动覆盖）',
      ...
      compactModeDesc:
        'Compact 模式是 OpenAI Responses API 的上下文压缩功能，控制本账号在 /responses/compact 调度中的参与方式。Auto 跟随探测结果，Force On 强制允许，Force Off 强制排除。',
      compactModeAuto: '自动',
    ...
    billingRateMultiplier: '账号成本倍率',
    billingRateMultiplierHint: '用于管理员侧账号成本、利润统计和利润保护，不直接影响用户扣费；0 表示该账号成本按 0 计算。',
    userBillingPricing: {
      title: '账号级用户价格（仅 OpenAI API Key）',
      description: '用于同一分组内不同 OpenAI API Key 账号具有不同上游售价的场景。分组仍负责账号池调度，实际扣费按最终选中的账号计算。',
      priorityDescription: '基础价格：命中账号模型完整价格时优先使用，否则沿用现有价格链。倍率：用户个人倍率优先，其次账号级价格倍率，最后使用分组倍率。',
      multiplierTitle: '账号级价格倍率',
      multiplierDescription: '适用于上游也是中转站、按统一倍率加价的账号。启用后不要求逐模型配置完整价格。',
      multiplierLabel: '价格倍率',
      multiplierPlaceholder: '请填写倍率，例如 1.2',
      multiplierHint: '不启用时保持为空；启用后必须填写大于 0 的倍率。',
      modelPricingTitle: '账号级模型完整价格',
      modelPricingDescription: '按需逐个添加模型。只有模型代号精确命中时才覆盖现有基础价格，未配置的模型继续沿用现有价格。',
      addModelPricing: '添加模型价格',
      noModelPricing: '尚未配置账号级模型完整价格',
      modelRequired: '账号级模型价格必须填写至少一个模型代号',
      combinedWarning: '完整价格和账号级价格倍率将同时生效：命中模型完整价格后，以该价格为基础，再应用账号级倍率；用户个人倍率仍具有最高优先级。',
      multiplierRequired: '启用账号级价格倍率后，请填写大于 0 的倍率'
    },
    expiresAt: '过期时间',
