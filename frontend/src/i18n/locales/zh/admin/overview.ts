export default {
    ...
    groups: {
      ...
      form: {
        ...
        firstOutputFailover: '首 Token 超时切换',
        firstOutputFailoverEnabled: '启用 OpenAI API Key 号池首 Token 超时切换',
        firstOutputFailoverTimeoutLabel: '首 Token 等待时长（秒）',
        firstOutputFailoverMaxSwitchesLabel: '最多切换次数',
        firstOutputFailoverHint: '仅对 OpenAI API Key 号池流式请求生效。指定时间内没有有效首 Token 时切换账号；达到次数后，最后一个账号将继续等待。',
      ...
      videoPricing: {
        title: '视频生成计费',
        description:
          '配置视频生成的每秒单价（USD/秒），留空则使用系统默认价格。可按需添加模型代号，并为不同分辨率设置覆盖价格。',
        modelOverridesTitle: '按模型覆盖视频价格（可选）',
        modelOverridesDescription: '自行添加模型代号后，已填写的分辨率价格会覆盖该模型的通用分辨率价格；未填写的分辨率回退到通用价格。',
        addModel: '添加模型覆盖',
        removeModel: '移除模型',
