export default {
    settings: {
      ...
      site: {
        ...
        homeContentHint: 'Customize the home page content. In HTML mode, wrap variable names with two opening and two closing braces. Available variables: site_name, site_logo, site_subtitle, api_base_url, contact_info, doc_url, and version. If you enter a URL (starting with http:// or https://), it will be used as an iframe src to embed an external page. When set, the default status information will no longer be displayed.',
        homeContentIframeWarning: '⚠️ iframe mode note: Some websites have X-Frame-Options or CSP security policies that prevent embedding in iframes. If the page appears blank or shows an error, please verify the target website allows embedding, or consider using HTML mode to build your own content.',
        homeScript: 'Custom Home Script',
        homeScriptPlaceholder: 'Enter custom home JavaScript, for example: document.querySelector("[data-locale=zh]")?.addEventListener("click", () => window.Sub2API?.setLocale("zh"))',
        homeScriptHint: 'Loaded only in HTML mode after the custom HTML is rendered. The script can use window.Sub2API.getLocale(), setLocale(locale), and getPublicSettings(). It has the current page\'s frontend privileges, so only enter trusted code.',
