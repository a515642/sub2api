...
// 本地设计：openai_responses_mode 必须显式选择（'' 表示未选择，提交时强制）。
...
const normalizeOpenAIEndpointCapabilities = (values: OpenAIEndpointCapability[]) => {
  const allowed: OpenAIEndpointCapability[] = ['chat_completions', 'embeddings', 'video_generation', 'seedance']
...
const submitCreateAccount = async (payload: CreateAccountRequest) => {
  ...
  try {
    ...
    const modelMapping = finalPayload.credentials.model_mapping
    const hasConcreteMappedTarget = finalPayload.type === 'apikey' &&
